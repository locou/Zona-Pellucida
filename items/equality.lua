...
function equality:OnRoomClear()
  ...
  if(ply:HasCollectible(Equality.ID)) then
    ...
    if(room:IsFirstVisit() and room:IsClear() and reward) then
      ...
      local chance = (1 - math.sqrt(((math.abs(coins-bombs)+math.abs(coins-keys)+math.abs(keys-bombs))/198))) * .2
      if(math.random() < chance) then
          local chest = game:Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_CHEST, room:FindFreePickupSpawnPosition(ply.Position, 5.0, true), Vector(0,0), ply, 2, RNG():GetSeed())
      end
      reward = false
    end
  end
